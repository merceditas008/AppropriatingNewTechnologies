...
void testApp::setup() {
    ...
    ///////////load the soundfile
    beats.loadSound("click1.wav");
    
    //////////initialize all the variables we will use to trigger the effects of the "blinkiness"
    show_me = false;
    justBlinked = false;
    timeWait=50;
    timeBlinked=0;
...
void testApp::update() {
	...
	if(cam.isFrameNew()) {
		...
		if(tracker.getFound()) {
            ...
            //////////////////WE LOOK AT THE CURVE AND SET A THRESHOLD TO DETECT THE BLINKS TRYING TO MAKE IT MORE ACCURATE
            //cout<< lastavg - avg << endl;
...
void testApp::draw() {
	...
	ofSetColor(255);
    if (justBlinked){
       timeBlinked= ofGetElapsedTimeMillis();
    }
    
    
    int timeSinceBlinked = ofGetElapsedTimeMillis() - timeBlinked;
    
    if (timeSinceBlinked < timeWait){
        show_me = true;
        beats.play();
